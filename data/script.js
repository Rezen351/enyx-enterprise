        ...
        if (editModbusIdx === idx) {
            ...
            m.registers.forEach((r, ridx) => {
                html += `<div class="register-row">
                    <div><label class="text-xs" style="margin-bottom:4px; display:block;">Address</label><input type="number" placeholder="Register Address" value="${r.address}" onchange="hwModbus[${idx}].registers[${ridx}].address=parseInt(this.value)" class="form-input-sm"></div>
                    <div><label class="text-xs" style="margin-bottom:4px; display:block;">Type</label><select onchange="hwModbus[${idx}].registers[${ridx}].type=this.value" class="form-select-sm"><option value="HOLDING" ${r.type === 'HOLDING' ? 'selected' : ''}>HOLDING (03)</option><option value="INPUT" ${r.type === 'INPUT' ? 'selected' : ''}>INPUT (04)</option></select></div>
                    <div style="grid-column: 1 / -1;"><label class="text-xs" style="margin-bottom:4px; display:block;">Name</label><input type="text" placeholder="Register Name" value="${escapeHtml(r.name)}" onchange="hwModbus[${idx}].registers[${ridx}].name=this.value" class="form-input-sm"></div>
                    <div><label class="text-xs" style="margin-bottom:4px; display:block;">Length</label><select onchange="hwModbus[${idx}].registers[${ridx}].length=parseInt(this.value)" class="form-select-sm" title="Register Count"><option value="1" ${(r.length || 1) == 1 ? 'selected' : ''}>1 reg</option><option value="2" ${(r.length || 1) == 2 ? 'selected' : ''}>2 reg</option></select></div>
                    <div><label class="text-xs" style="margin-bottom:4px; display:block;">Data Type</label><select onchange="hwModbus[${idx}].registers[${ridx}].data_type=this.value" class="form-select-sm" title="Data Type"><option value="UINT16" ${(r.data_type || 'UINT16') === 'UINT16' ? 'selected' : ''}>UINT16</option><option value="INT16" ${(r.data_type || 'UINT16') === 'INT16' ? 'selected' : ''}>INT16</option><option value="FLOAT32" ${(r.data_type || 'UINT16') === 'FLOAT32' ? 'selected' : ''}>FLOAT32</option><option value="INT32" ${(r.data_type || 'UINT32') === 'INT32' ? 'selected' : ''}>INT32</option><option value="UINT32" ${(r.data_type || 'UINT16') === 'UINT32' ? 'selected' : ''}>UINT32</option></select></div>
                    <div><label class="text-xs" style="margin-bottom:4px; display:block;">Multiplier</label><input type="number" step="0.01" placeholder="Multiplier" value="${r.multiplier}" onchange="hwModbus[${idx}].registers[${ridx}].multiplier=parseFloat(this.value)" class="form-input-sm"></div>
