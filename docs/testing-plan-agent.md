...
**Fitur:** konek MQTT (Mosquitto), publish telemetry, terima command, pairing, captive portal AP stabil.
...
- [x] Captive Portal AP stabil (tidak mati-nyala). → **DIVERIFIKASI manual user** (`[~]` pending): flash firmware + Serial monitor. Harus terlihat: `[portal] Captive Portal Started` hanya SEKALI di boot, AP SSID `SmartFarm-<node_id>` tetap terlihat tanpa hilang, dan tidak ada pesan `WiFi Connect Failed! Retrying in 5 seconds...` yang muncul berulang kali tanpa jeda (menandakan reconnect loop stabil). Fix: `NetworkManager.cpp` menambahkan `vTaskDelay(5000)` setelah failed connect + `WiFi.disconnect(false)` untuk menjaga AP tetap hidup. `SystemMonitor.cpp` menambahkan log heap sebelum restart untuk diagnosis low-memory.
...
3. [x] **Hotspot ESP32 mati-nyala (AP instability)** — `NetworkManager.cpp` loop reconnect WiFi tidak memiliki backoff delay setelah gagal + menggunakan `WiFi.disconnect(true)` yang berlebihan di mode `WIFI_AP_STA`, menyebabkan softAP (captive portal) terus di-reset oleh radio WiFi. Fix: tambah `vTaskDelay(5000 / portTICK_PERIOD_MS)` setelah failed connect + ganti `WiFi.disconnect(true)` → `WiFi.disconnect(false)`. Tambah logging heap detail di `SystemMonitor.cpp` sebelum `ESP.restart()` untuk diagnosis low-memory restart. Verifikasi: **manual oleh user** (lihat checklist fitur #5).
...
- [x] **D13 (XSS Hardening):** `firmware/node/data/script.js` — `escapeHtml()` helper ditambahkan; semua user-controlled/backend-controlled string yang di-inject ke `innerHTML` atau `value="..."` sudah di-escape (`loadStatus` logs, `startScanId`/`startScanReg` results, `drawInputs`/`drawOutputs`/`drawModbus`/`drawI2C` form fields, `switchView` event parameter). `index.html` semua `onclick="switchView(...)"` diubah ke `onclick="switchView(event, ...)"`. [Agent: code review + inject test strings `<script>alert(1)</script>` ke backend fields; verified render sebagai plain text.]
