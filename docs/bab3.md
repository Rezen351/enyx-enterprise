...
**Cara Kerja Keseluruhan (Narrative)**

Cara kerja sistem FreeRTOS pada firmware ini dapat dipahami sebagai sebuah orkestra dengan dua pemain utama yang bekerja secara paralel. Di Core 0, WiFiTask berperan sebagai penjaga gerbang: ia terus-menerus memantau koneksi internet, dan jika koneksi hilang, ia secara otomatis beralih ke mode Access Point untuk menyiapkan captive portal yang memungkinkan pengguna mengkonfigurasi perangkat melalui browser. Di Core 0 juga ada MqttTask yang berfungsi sebagai penerjemah pesan: ia menerima perintah aktuator dari backend melalui broker MQTT dan meneruskannya ke antrean eksekusi, sekaligus mempublikasikan telemetri yang diterima dari Core 1. WatchdogTask hadir sebagai pengawas keamanan yang menerima sinyal heartbeat dari semua task; jika suatu task berhenti mengirimkan heartbeat, watchdog akan langsung melakukan restart task tersebut atau memicu reset sistem. SysMonitorTask dan DiscoveryPeriodic juga berjalan di Core 0, masing-masing memantau kesehatan memori dan memastikan node terdeteksi oleh sistem setiap 60 detik. Sementara itu, Core 1 dialokasikan sepenuhnya untuk TelemetryTask yang setiap 5 detik membaca seluruh sensor—baik GPIO digital/analog, Modbus RS485, maupun I2C—menggabungkan hasilnya menjadi JSON, dan meneruskannya ke MqttTask untuk dipublikasikan. Karena setiap task memiliki prioritas, scheduler FreeRTOS akan menjalankan task dengan prioritas lebih tinggi lebih dulu; jika terjadi konflik, task dengan prioritas lebih rendah bisa ditunda sebentar tanpa membahayakan operasi. Pendekatan ini memastikan bahwa kegagalan satu komponen—misalnya WiFi putus—tidak akan memblokir pembacaan sensor atau eksekusi aktuator, sehingga sistem tetap andal dan responsif.
...
#### F. Dual-Partition OTA Update dengan Rollback Otomatis
